                    ...
                    {activeCategory && projectsData[activeCategory] ? (
                        projectsData[activeCategory].map((item, index) => (
                            <div key={index} className="p-4 bg-background rounded-lg border border-white/10 space-y-3">
                                <div className="flex justify-between items-center"><h4 className="font-semibold text-lg text-primary">{item.title || `Item ${index + 1}`}</h4><RemoveButton onClick={() => updateState([activeCategory], projectsData[activeCategory].filter((_, i) => i !== index))}>Remove Item</RemoveButton></div>
